...
1\ )\rbrack$)
\Cancel\ \command{\hado\ +\ $D$}
\migi\ \command{\gyakusyoryu\ +\ $D$}\\\hline
補足&最初に覚えるべき基礎コンボ2。スタン値重視\\\bhline{2pt}
\fi
コンボ&
...
1\ )\rbrack$)
\Cancel\ \command{\hado\ +\ $D$}\\
&\command{\gyakusyoryu\ +\ $B$\ }\ \migi \ \command{\gyakusyoryu\ +\ $BD$}
\\\hline
...
1\ )\rbrack$)
\Cancel\ \command{\hado\ +\ $D$}\\
&\command{\gyakusyoryu\ +\ $B$\ }\ \migi \ \command{\orochi\ +\ $AC$}
\\\hline
補足&倒しきれる時用。タメ不要。端ではダメージ減\\\bhline{2pt}
コンボ&
\command{\downarrow $B$}\ $\times\ 1\ \sim\ 2$ \Cancel \command{立$B$}
\Cancel\ \command{\hado\ +\ $B$}\\
&\command{\orochi\ +\ $AC$}\\\hline
補足&倒しきれる時用の小技始動コンボ。タメ不要。端ではダメージ減\\\bhline{2pt}
\end{tabular*}
\endgroup
\if0
\begin{classify}{$\lbrack\ $近\ $C\ \rbrack$\ \Cancel\ $\lbrack\ \searrow\ D\ (\
1\ )\rbrack$} \class{ \begin{classify}{
			\hspace{-12pt}
			\begin{tabular}[t]{l}
			...
			\class{
				...
				\begin{tabular}[t]{|lll|l|l|}
					...
					\hline
					\starthree&0PG&0\% &Damage&Stan\\
					\hline
					&&&275&18\\
						\hline\multicolumn{5}{l}{
						一番最初に覚えるべき基礎コン。迷ったらコレ
					}\\
					\multicolumn{5}{l}{
...
\end{classify}
\newpage
\begin{classify}{$\lbrack\ \downarrow\ B\ \rbrack\ \times\ 1\ \sim\ 2$\
\Cancel\ $\lbrack$\ 立\ $B\ \rbrack$} \class{
		\begin{classify}{
				\hspace{-12pt}
					\begin{tabular}[t]{|lll|l|l|}
						\multicolumn{5}{l}{
					...
					\hline
					\multicolumn{5}{l}{
						安いが簡単で状況も悪く無いので絶対覚える
					}\\
					\multicolumn{5}{l}{
						無敵技・1F投げを持たない相手にはほぼノーリスク
					}
					\end{tabular}
		}
			%%一つのクラス
			...
			\class{
				\begin{classify}{
					...
					\begin{tabular}[t]{|lll|l|l|}
						...
						\hline
						\starthree&2PG&0\% &Damage&Stan\\
						\hline
						&&&235&23\\
						\hline
						\multicolumn{5}{l}{
							端で弱75式が入れば必ず追撃する。ドライブがあればここから伸ばす%%説明
						}
					\end{tabular}
					}
					%%一つのクラス
...
$B$\ or\ $D$}\ \Cancel\ \command{\downarrow\ \uparrow\ +\ $B$\ or\
$D$\ (\ 1\ )\ }\\
&\DC\ \command{\tenti\ $AC$}\ \migi\ \command{\tatsu\ +\ $A$\ or\ $C$}\\
&\migi\ \command{\hado\ +\
$B$\ or\ $D$}\ \Cancel\ \command{\downarrow\ \uparrow\ +\ $B$\ or\
$D$\ }\\\hline
補足&上記２つのコンボの複合系。
...
%%一つのクラス
\if0
\begin{classify}{$\lbrack\ $近\ $\ D\ \rbrack$}
	\class{
		\hspace{-12pt}
		\begin{tabular}[t]{|lll|l|l|}
			\multicolumn{5}{l}{
			\Cancel \ \ \lbrack\ \ \tenti\ A$\ or\ $C\ \ \rbrack\ \ 
			%%→紅コレ
			}\\
			\hline
			\startwo&2PG&50\% &Damage&Stan\\
	...
	\class{
		\begin{classify}{
			...
			\begin{tabular}[t]{|lll|l|l|}
				\multicolumn{5}{l}{
				\Cancel \ \ \lbrack\ \ \hado\ B$\ or\ $D\ \ \rbrack\ \ 
				%%→居合い蹴り
				}
			\end{tabular}
		}
